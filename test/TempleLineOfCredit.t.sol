...
//     function testAddDebtTokenFailsAlreadyExists() public {
//         vm.startPrank(admin);
//         tlc.addDebtToken(address(daiToken), daiTokenType, daiInterestRateBps, daiPrice, daiMinCollateralizationRatio);
//         vm.expectRevert(abi.encodeWithSelector(TempleLineOfCredit.AlreadyExists.selector, address(daiToken)));
//         tlc.addDebtToken(address(daiToken), daiTokenType, daiInterestRateBps, daiPrice, daiMinCollateralizationRatio);
//         vm.stopPrank();
//     }
...
//         vm.prank(admin);
//         tlc.addDebtToken(address(daiToken), daiTokenType, daiInterestRateBps, daiPrice, daiMinCollateralizationRatio);
//         (, uint256 interestRateBps, uint256 interestRateLastUpdatedAt, uint256 interestRateAccuredStored, TempleLineOfCredit.TokenPrice tokenPrice, uint256 minCollateralizationRatio, bool allowed) = tlc.debtTokens(address(daiToken));
...
//         assertEq(interestRateAccuredStored, 0);
//         assertEq(interestRateLastUpdatedAt, block.timestamp);
...
//     function testSetInterestRateBpsUnsupported() public {
//         vm.expectRevert(abi.encodeWithSelector(TempleLineOfCredit.Unsupported.selector, address(0x0)));
//         vm.prank(admin);
//         tlc.setInterestRateBps(address(0x0), 0);
//     }

//     function testSetInterestRateBpsSucess() public {
//         vm.prank(admin);
//         tlc.addDebtToken(address(daiToken), daiTokenType, daiInterestRateBps, daiPrice, daiMinCollateralizationRatio);
//         uint256 currentTimeStamp = block.timestamp;
//         vm.warp(1 days);
//         vm.prank(admin);
//         uint256 newInterestRateBps = 600;
//         tlc.setInterestRateBps(address(daiToken), newInterestRateBps);
//         (,uint256 interestRateBps, uint256 interestRateLastUpdatedAt, uint256 interestRateAccuredStored,,,) = tlc.debtTokens(address(daiToken));
//         assertEq(newInterestRateBps, interestRateBps);
//         assertEq(interestRateLastUpdatedAt, block.timestamp);
//         assertEq(daiInterestRateBps * (block.timestamp - currentTimeStamp) , interestRateAccuredStored);
//     }
...
//         // deposit DAI
//         (,,,,,, bool allowedDai) = tlc.debtTokens(address(daiToken));
//         if (!allowedDai) {
//            tlc.addDebtToken(address(daiToken), TempleLineOfCredit.TokenType.TRANSFER, daiInterestRateBps, daiPrice, daiMinCollateralizationRatio);
//         }
...
//         // support OUD
//          (,,,,,, bool allowedOud) = tlc.debtTokens(address(oudToken));
//         if (!allowedOud) {
...
//         assertEq(tpDAI.principalAmount, maxBorrowCapacityDAI);
...
//         assertEq(tpOUD.principalAmount, maxBorrowCapacityOUD);
...
//         assertEq(borrowAmount, tlc.getTotalDebtAmount(address(daiToken), alice));
//         assertEq(borrowAmount, tlc.getTotalDebtAmount(address(oudToken), alice));
...
//         assertEq(tlc.getTotalDebtAmount(address(daiToken), alice), borrowDAIAmountFirst + ((borrowDAIAmountFirst * daiInterestRateBps * secondsElapsed)  / 10000 / 365 days) + borrowDAIAmountSecond );
//         assertEq(daiToken.balanceOf(alice), borrowDAIAmountFirst + borrowDAIAmountSecond);
        
//         assertEq(tlc.getTotalDebtAmount(address(oudToken), alice), borrowOUDAmountFirst + ((borrowOUDAmountFirst * oudInterestRateBps * secondsElapsed)  / 10000 / 365 days) + borrowOUDAmountSecond );
//         assertEq(oudToken.balanceOf(alice), borrowOUDAmountFirst + borrowOUDAmountSecond);
//     }


//     function testBorrowWithChangingInterestRateSucess(uint32 firstTimeElapsed, uint32 secondTimeElapsed) external {
//         uint256 borrowDAIAmountFirst = uint(30_000e18);
//         uint256 borrowOUDAmountFirst = uint(20_000e18);
        
//         _borrow(alice, uint(100000_00e18), borrowDAIAmountFirst, borrowOUDAmountFirst);

//         // uint firstBorrowSecondsElapsed = 200 days;
//         vm.warp(block.timestamp +  firstTimeElapsed);

//         uint256 newDaiInterestRate = 700; // 7%
//         uint256 newOUDInterestRate = 900; // 9%

//         vm.startPrank(admin);
//         tlc.setInterestRateBps(address(daiToken), newDaiInterestRate);
//         tlc.setInterestRateBps(address(oudToken), newOUDInterestRate);
//         vm.stopPrank();

//         uint256 borrowDAIAmountSecond = uint(30_000e18);
//         uint256 borrowOUDAmountSecond = uint(20_000e18);

//         uint256 firstBorrowTotalUserDebt = tlc.getTotalDebtAmount(address(daiToken), alice);

//         _borrow(alice, uint(100000_000e18), borrowDAIAmountSecond, borrowOUDAmountSecond);

//         vm.warp(block.timestamp +  secondTimeElapsed);

//         uint256 expectedFirstBorrowTotalUserDebt = borrowDAIAmountFirst +  ((borrowDAIAmountFirst * daiInterestRateBps * firstTimeElapsed) / 10000 / 365 days); // First borrow accured debt    
//         uint256 finalexpectedTotalDaiDebt = expectedFirstBorrowTotalUserDebt + borrowDAIAmountSecond + ((((borrowDAIAmountFirst + borrowDAIAmountSecond)) * newDaiInterestRate * secondTimeElapsed) / 10000 / 365 days); // Total accured interest after the second borrow

//         assertEq(expectedFirstBorrowTotalUserDebt, firstBorrowTotalUserDebt);
//         assertEq(finalexpectedTotalDaiDebt, tlc.getTotalDebtAmount(address(daiToken), alice));
//     }
...
//         TempleLineOfCredit.DebtPosition memory tpDAI = tlc.getDebtPosition(address(daiToken), alice);
//         assertEq(borrowDAIAmount - repayDAIAmount,  tpDAI.principalAmount);

//         TempleLineOfCredit.DebtPosition memory tpOUD = tlc.getDebtPosition(address(oudToken), alice);
//         assertEq(borrowOUDAmount - repayOUDAmount, tpOUD.principalAmount);
//     }


//     function testRepayGreaterThanPrincipleSucess() external {
//         uint256 borrowDAIAmount = uint(60_000e18);
        
//         _borrow(alice, uint(100_000e18), borrowDAIAmount, borrowDAIAmount);

//         vm.warp(10 days);

//         vm.startPrank(alice);
//         deal(address(daiToken), alice, uint(100_000e18));
//         daiToken.approve(address(tlc), uint(100_000e18));
//         address[] memory debtTokens = new address[](1);
//         debtTokens[0] = address(daiToken);
//         uint256[] memory repayAmounts = new uint256[](1);
//         repayAmounts[0] = tlc.getTotalDebtAmount(address(daiToken), alice);
//         tlc.repay(debtTokens, repayAmounts);
//         vm.stopPrank();

//         TempleLineOfCredit.DebtPosition memory tpDAI = tlc.getDebtPosition(address(daiToken), alice);
//         assertEq(tpDAI.principalAmount, 0);
//         assertEq(tpDAI.interestAmount,  0);
...
//         TempleLineOfCredit.DebtPosition memory tpDAI = tlc.getDebtPosition(address(daiToken), alice);
//         assertEq(0,  tpDAI.principalAmount);

//         TempleLineOfCredit.DebtPosition memory tpOUD = tlc.getDebtPosition(address(oudToken), alice);
//         assertEq(0, tpOUD.principalAmount);
//     }
